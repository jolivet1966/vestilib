...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    mapInstance.current = new window.google.maps.Map(mapRef.current, {
      center: { lat: 46.5, lng: 2.5 }, zoom: 3,
      styles: [
  ...
  return (
    <div className="flex flex-col bg-gray-50" style={{ height: '100dvh' }}>
      ...
      {/* Header */}
      <div className="bg-[#272757] px-4 py-3 flex items-center gap-3 flex-shrink-0 shadow-lg">
          ...
          {!mapsReady && (
            <div className="absolute inset-0 flex items-center justify-center bg-gray-100">
              <div className="text-center">
                <div className="w-8 h-8 border-4 border-[#272757] border-t-transparent rounded-full animate-spin mx-auto mb-2" />
        ...
        <div className="flex-1 md:flex-none md:w-80 overflow-y-auto bg-gray-50 order-last md:order-first">
          {loading ? (
            <div className="flex items-center justify-center h-32">
              <div className="w-6 h-6 border-3 border-[#272757] border-t-transparent rounded-full animate-spin" />
            </div>
          ) : hosts.length === 0 ? (
            ...
            <div className="p-3 space-y-2 pb-24">
              ...
              <div className="flex items-center gap-2 px-1 py-2">
                <div className="w-6 h-6 bg-[#272757] rounded-lg flex items-center justify-center">
                  <span className="text-[#F5C84A] text-[10px] font-black">{hosts.length}</span>
                </div>
                <p className="text-xs font-semibold text-[#272757]">
                ...
                return (
                  ...
                  onClick={() => setSelectedHost(isSelected ? null : host)}
                    className={`rounded-2xl border cursor-pointer transition-all overflow-hidden ${
                      isSelected
                        ? 'border-[#272757] shadow-md shadow-[#272757]/10'
                        : 'border-gray-100 bg-white hover:shadow-sm hover:border-gray-200'
                    }`}>

                    {/* Bande colorée en haut si sélectionné */}
                    {isSelected && <div className="h-1 bg-gradient-to-r from-[#272757] to-[#F5C84A]" />}

                    <div className={`p-4 ${isSelected ? 'bg-white' : ''}`}>
                      {/* Nom + badge */}
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-sm font-bold text-[#272757]">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {host.prestations?.slice(0, 3).map(pid => {
                          ...
                          return t ? (
                            <span key={pid} className="text-[10px] bg-[#272757]/8 text-[#272757] px-2.5 py-1 rounded-full font-medium border border-[#272757]/10">
                              {t.label}
                            </span>
                          ) : null
                        })}
                      ...
                      {isSelected && (
                        <Link href={`/host/${host.id}`}
                          className="mt-4 flex items-center justify-center gap-2 w-full bg-[#272757] text-[#F5C84A] text-sm font-bold py-3 rounded-xl hover:bg-[#0C2447] transition-colors active:scale-95">
