    ...
    for (const host of hosts) {
      geocoder.geocode({ address: `${host.adresse}, ${host.codePostal} ${host.ville}, France` }, (results: any, status: any) => {
        ...
        const marker = new window.google.maps.Marker({
          ...
          icon: { path: window.google.maps.SymbolPath.CIRCLE, scale: 12, fillColor: '#1A3A6B', fillOpacity: 1, strokeColor: '#F5C84A', strokeWeight: 2 },
        })
        marker.addListener('click', () => { setSelectedHost(host); mapInstance.current.panTo(position); mapInstance.current.setZoom(15) })
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      <div className="bg-[#1A3A6B] px-4 py-3 flex items-center gap-4 flex-shrink-0">
        ...
        <div className="flex-1 relative">
          <input ref={inputRef} type="text" placeholder="Rechercher une ville, une adresse..."
