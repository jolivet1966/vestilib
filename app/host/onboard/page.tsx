    ...
    <div className="min-h-screen bg-gray-50 flex items-start justify-center py-10 px-4">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-[#272757] tracking-widest mb-1">VESTILIB</h1>
          ...
          {[1,2,3].map(n => (
            <div key={n} className="flex items-center gap-2 flex-1">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold flex-shrink-0 transition-colors ${
                ...
                etape === n ? 'bg-[#272757] text-[#F5C84A]' :
                'bg-gray-200 text-gray-400'
              }`}>
                {etape > n ? 'ok' : n}
              </div>
              <span className={`text-xs font-medium hidden sm:block ${etape === n ? 'text-[#272757]' : 'text-gray-400'}`}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          {etape === 1 && (
            <div>
              ...
              <div className="space-y-4">
                <div className="mb-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => setTypeCompte('individual')}
                      className={`p-3 rounded-xl border text-left transition-colors ${typeCompte === 'individual' ? 'border-[#272757] bg-[#272757]/5' : 'border-gray-100'}`}>
                    ...
                    <button type="button" onClick={() => setTypeCompte('company')}
                      className={`p-3 rounded-xl border text-left transition-colors ${typeCompte === 'company' ? 'border-[#272757] bg-[#272757]/5' : 'border-gray-100'}`}>
                ...
                <div>
                  ...
                  <input ref={adresseRef} type="text" value={adresse} onChange={e => setAdresse(e.target.value)}
                    ...
                    className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#272757] transition-colors" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
              ...
              <button onClick={validerEtape1} className="mt-6 w-full bg-[#272757] text-[#F5C84A] font-medium py-3 rounded-xl hover:bg-[#0C2447] transition-colors">
                Continuer
              </button>
            </div>
          )}
          ...
          {etape === 2 && (
            <div>
                  ...
                  <div key={jour} className="py-2.5 border-b border-gray-50 last:border-0">
                    <div className="flex items-center gap-2 mb-2">
                      <input type="checkbox" checked={horaires[jour].ouvert}
                        ...
                        className="w-4 h-4 accent-[#272757] flex-shrink-0" />
                    ...
                    {horaires[jour].ouvert ? (
                      <div className="flex items-center gap-2 ml-6">
                        <input type="time" value={horaires[jour].ouverture}
                          ...
                          className="flex-1 border border-gray-200 rounded-xl px-2 py-2 text-sm text-center focus:outline-none focus:border-[#272757] bg-gray-50" />
              ...
              <div className="mt-6 flex gap-3">
                ...
                <button onClick={validerEtape2} className="flex-1 bg-[#272757] text-[#F5C84A] font-medium py-3 rounded-xl hover:bg-[#0C2447] transition-colors">
                  Continuer
                </button>
              </div>
            </div>
          ...
          {etape === 3 && (
            <div>
                      ...
                      <div className="space-y-2">
                        {tarifs.map(tarif => (
                          <label key={tarif.id} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-colors ${
                            prestations.includes(tarif.id) ? 'border-[#272757] bg-[#272757]/5' : 'border-gray-100 hover:border-gray-200'
                          }`}>
                            <div className="flex items-center gap-3">
                              <input type="checkbox" checked={prestations.includes(tarif.id)}
                                onChange={() => togglePrestation(tarif.id)}
                                className="w-4 h-4 accent-[#272757]" />
                              <div>
                                <p className="text-sm font-medium text-gray-800">{tarif.label}</p>
                                <p className="text-xs text-gray-400">{tarif.description}</p>
                              </div>
                            </div>
                            <span className={`text-sm font-semibold flex-shrink-0 ml-2 ${tarif.prix < 0 ? 'text-green-600' : 'text-[#272757]'}`}>
              ...
              {prestations.length > 0 && (
                <div className="mt-4 bg-[#272757]/5 rounded-xl p-3">
              ...
              <div className="mt-5 border-t border-gray-100 pt-5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setModeReservation('immediat')}
                    className={`p-3 rounded-xl border text-left transition-colors ${modeReservation === 'immediat' ? 'border-[#272757] bg-[#272757]/5' : 'border-gray-100'}`}>
                  ...
                  <button type="button" onClick={() => setModeReservation('validation')}
                    className={`p-3 rounded-xl border text-left transition-colors ${modeReservation === 'validation' ? 'border-[#272757] bg-[#272757]/5' : 'border-gray-100'}`}>
              ...
              <div className="mt-6 flex gap-3">
                ...
                <button onClick={soumettre} disabled={loading}
                  className="flex-1 bg-[#272757] text-[#F5C84A] font-medium py-3 rounded-xl hover:bg-[#0C2447] disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
    ...
    <div className="bg-gray-50 rounded-xl p-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 text-center">
          <p className="text-3xl font-black text-[#272757]">{value}</p>
          <p className="text-xs text-gray-400">{unite} max / creneau</p>
        </div>
        <button onClick={() => onChange(value + 1)}
          className="w-9 h-9 rounded-full bg-[#272757] text-[#F5C84A] text-lg flex items-center justify-center hover:bg-[#0C2447]">+</button>
      </div>
    </div>
  )
}
    ...
    <div>
      ...
      <div className="relative">
        <input type={inputType} value={value} onChange={e => onChange(e.target.value)}
          ...
          className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#272757] transition-colors pr-10" />
        {type === 'password' && (
