...
useEffect(() => {
  ...
  if (authChecked && !isConnected) {
    return (
      <div className="min-h-screen bg-[#272757] flex flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <div className="flex flex-col gap-3 w-full max-w-xs mt-4">
          <Link href={`/user/login?redirect=/host/${hostId}`}
            className="w-full bg-[#F5C84A] text-[#272757] font-bold py-3.5 rounded-2xl text-center">
            Se connecter
          </Link>
  ...
  if (error || !host) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4">
      ...
      <Link href="/map" className="text-[#272757] text-sm underline">Retour à la carte</Link>
    </div>
  )
  ...
  if (host.ouvert === false) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4 px-6 text-center">
      ...
      <Link href="/map" className="bg-[#272757] text-[#F5C84A] font-semibold px-8 py-3 rounded-xl">
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-1">
            {JOURS_ORDER.map(jour => {
              ...
              return (
                <div key={jour} className={`text-center rounded-lg p-1.5 ${h?.ouvert ? 'bg-[#272757]/5' : 'bg-gray-50'}`}>
                  <p className="text-[9px] font-semibold text-gray-400 mb-0.5">{JOURS_LABELS[jour]}</p>
                  {h?.ouvert ? (
                    <><p className="text-[8px] text-[#272757] font-medium leading-tight">{h.ouverture}</p><p className="text-[8px] text-[#272757] leading-tight">{h.fermeture}</p></>
        ...
        <Link href={`/messages?hostId=${host.id}`} className="flex items-center justify-center gap-1.5 text-[#272757]/60 text-xs py-2 hover:text-[#272757] transition-colors w-full">
  ...
  Contacter l&apos;hote
        ...
        <div className="flex gap-2 mb-5">
          {[{n:1,label:'Prestations'},{n:2,label:'Date & Heure'},{n:3,label:'Paiement'}].map(s => (
            <div key={s.n} className={`flex-1 text-center py-2.5 rounded-xl text-xs font-semibold border transition-all ${
              etape === s.n ? 'bg-[#272757] text-[#F5C84A] border-[#272757] shadow-sm' :
        ...
        {etape === 1 && (
          <div className="space-y-4">
                      ...
                      return (
                        <div key={tarif.id}
                          ...
                          className={`flex items-center gap-3 p-4 rounded-2xl border transition-all cursor-pointer ${
                            selected
                              ? 'border-[#272757] bg-white shadow-sm'
                              : 'border-gray-100 bg-white hover:border-gray-200'
                          }`}>
                          {/* Checkbox visuel */}
                          <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                            selected ? 'bg-[#272757] border-[#272757]' : 'border-gray-300'
                          }`}
                          ...
                          <div className="flex items-center gap-3 flex-shrink-0">
                            {selected && (
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-bold text-[#272757] w-4 text-center">{qty}</span>
                                <button onClick={e => { e.stopPropagation(); updateQty(tarif.id, 1) }}
                                  className="w-7 h-7 rounded-full bg-[#272757] text-[#F5C84A] font-bold text-base flex items-center justify-center hover:bg-[#0C2447] active:scale-90 transition-all">
                                  +
                                </button>
                              </div>
                            )}
                            <span className={`text-base font-black min-w-[48px] text-right ${tarif.prix < 0 ? 'text-green-500' : 'text-[#272757]'}`}>
            ...
            {/* Récap total */}
            {total > 0 && !hasDepot && (
              <div className="bg-[#272757] rounded-2xl p-4">
            ...
            {!hasDepot && (
              <button onClick={() => { if (total <= 0 || capaciteDepasse) return; setEtape(2) }}
                ...
                className="w-full bg-[#272757] text-[#F5C84A] font-bold py-4 rounded-2xl hover:bg-[#0C2447] disabled:opacity-30 disabled:cursor-not-allowed transition-all active:scale-95 text-sm shadow-lg shadow-[#272757]/20">
        ...
        {etape === 2 && (
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              <p className="text-sm font-bold text-[#272757] mb-3">Choisissez la date</p>
                ...
                {dates.map(d => (
                  <button key={d.date}
                    ...
                    className={`p-3.5 rounded-xl border text-left transition-all active:scale-95 ${
                      selectedDate === d.date
                        ? 'border-[#272757] bg-[#272757] shadow-sm'
            ...
            {selectedDate && creneaux.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                <p className="text-sm font-bold text-[#272757] mb-3">Choisissez un créneau</p>
                  ...
                  {creneaux.map(c => (
                    <button key={c}
                      ...
                      className={`py-3 px-2 rounded-xl border text-xs font-semibold transition-all active:scale-95 ${
                        selectedCreneau === c
                          ? 'border-[#272757] bg-[#272757] text-[#F5C84A]'
            ...
            {!autreCreneauMode ? (
              <button onClick={() => setAutreCreneauMode(true)}
                className="w-full border border-dashed border-[#272757]/30 text-[#272757] font-semibold py-3 rounded-2xl text-sm hover:bg-[#272757]/5 transition-all">
                + Demander un autre créneau
              </button>
            ) : (
              <div className="bg-white rounded-2xl border border-[#272757]/20 p-4 shadow-sm space-y-3">
                <p className="text-sm font-bold text-[#272757]">Demander un créneau personnalisé</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="time" value={autreCreneauDebut} onChange={e => setAutreCreneauDebut(e.target.value)}
                      className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#272757]" />
                  </div>
                  <div>
                ...
                {autreCreneauDebut && autreCreneauFin && (() => {
                  ...
                  return dureeMin > 0 ? (
                    <div className="bg-[#272757]/5 rounded-xl p-3">
                      <p className="text-xs text-[#272757] font-medium">
                ...
                <div>
                  ...
                  <textarea value={autreCreneauMessage} onChange={e => setAutreCreneauMessage(e.target.value)}
                    ...
                    rows={3}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#272757] resize-none" />
                </div>
                <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex-1 bg-[#272757] text-[#F5C84A] font-bold py-2.5 rounded-xl text-sm hover:bg-[#0C2447] disabled:opacity-40 transition-colors">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { if (!selectedDate || !selectedCreneau || isAnyComplet) return; setEtape(3) }}
                ...
                className="flex-1 bg-[#272757] text-[#F5C84A] font-bold py-4 rounded-2xl hover:bg-[#0C2447] disabled:opacity-30 disabled:cursor-not-allowed transition-all active:scale-95 shadow-lg shadow-[#272757]/20">
                Continuer →
              </button>
            </div>
          </div>
        ...
        {etape === 3 && (
          <div className="space-y-4">
            {/* Récap commande */}
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              <p className="text-sm font-bold text-[#272757] mb-3">Récapitulatif</p>
              ...
              <div className="bg-[#272757] rounded-xl p-3 mt-3 flex justify-between items-center">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              ...
              <input type="email" value={customerEmail} onChange={e => setCustomerEmail(e.target.value)}
                placeholder="vous@email.com"
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#272757] transition-colors" />
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={payer} disabled={paying || !customerEmail}
                className="flex-1 bg-[#272757] text-[#F5C84A] font-black py-4 rounded-2xl hover:bg-[#0C2447] disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-95 shadow-lg shadow-[#272757]/20 text-sm">
