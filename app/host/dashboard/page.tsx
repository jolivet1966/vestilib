  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async user => {
      ...
      try {
        ...
        const idTokenBal = await user.getIdToken()
          ...
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key as any)}
              className={`flex-1 min-w-[80px] py-3 text-xs font-semibold border-b-2 transition-all flex flex-col items-center gap-1 ${
                tab === t.key ? 'border-[#272757] text-[#272757]' : 'border-transparent text-gray-400 hover:text-gray-600'
              }`}>
              {t.icon}
              {t.label}
            </button>
                    ...
                    <div key={booking.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                      ...
                      <div className="p-4">
                        <div className="flex items-start justify-between gap-2 mb-3">
                          <div>
                            <p className="font-mono font-bold text-[#272757] text-base">{booking.bookingCode || '—'}</p>
                            ...
                            {refusBookingId === booking.id ? (
                              <div className="space-y-2">
                                <input type="text" value={motifRefus} onChange={e => setMotifRefus(e.target.value)}
                                  ...
                                  className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#272757]" />
                          ...
                          <div key={booking.id} className={`bg-white px-4 py-3 flex items-center justify-between ${i < bookingsArchivees.length - 1 ? 'border-b border-gray-100' : ''}`}>
                            ...
                            <div className="flex items-center gap-1.5">
                              <button onClick={() => desarchiverBooking(booking.id)} title="Restaurer"
                                className="w-8 h-8 rounded-lg bg-gray-50 border border-gray-200 hover:border-[#272757] flex items-center justify-center transition-colors">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2">
        ...
        {tab === 'solde' && (
          <div className="space-y-4">
            {balance ? (
              <>
                {/* Résumé solde */}
                <div className="bg-[#272757] rounded-2xl p-5 shadow-md relative overflow-hidden">
                ...
                <div className="bg-[#272757]/5 rounded-2xl p-4 border border-[#272757]/10 flex items-start gap-3">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#272757" strokeWidth="2" className="flex-shrink-0 mt-0.5">
                    <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
                  </svg>
                  <p className="text-xs text-[#272757] font-medium">Les virements sont effectues automatiquement chaque lundi via Stripe Connect.</p>
                </div>
              </>
            ) : (
              <div className="bg-white rounded-2xl p-10 text-center border border-gray-100 shadow-sm">
        ...
        {tab === 'services' && (
          <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        {tarifs.map(tarif => (
                          <label key={tarif.id} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${
                            ...
                            <span className={`text-sm font-bold flex-shrink-0 ml-2 ${tarif.prix < 0 ? 'text-emerald-600' : 'text-[#272757]'}`}>
            ...
            <button onClick={sauvegarderServices} disabled={saving}
              className="w-full bg-[#272757] text-[#F5C84A] font-bold py-3 rounded-2xl hover:bg-[#0C2447] disabled:opacity-50 transition-colors">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="date" value={dateDebut} onChange={e => setDateDebut(e.target.value)}
                      ...
                      className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#272757] bg-gray-50" />
                  </div>
                  <div>
                ...
                <button onClick={ajouterPeriodeFermeture} disabled={!dateDebut}
                  className="w-full bg-[#272757] text-[#F5C84A] px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-[#0C2447] disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
    ...
    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 text-center">
          <p className="text-2xl font-black text-[#272757] leading-none">{value}</p>
          <p className="text-xs text-gray-400 mt-1">{unite} max / creneau</p>
        </div>
        <button onClick={() => onChange(value + 1)}
          className="w-10 h-10 rounded-xl bg-[#272757] text-[#F5C84A] text-xl flex items-center justify-center hover:bg-[#0C2447] transition-colors font-bold">
          +
        </button>
      </div>
    </div>
