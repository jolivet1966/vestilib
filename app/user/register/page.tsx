'use client'
// app/user/register/page.tsx — Inscription utilisateur
...
export default function UserRegisterPage() {
  ...
  const inscrire = async () => {
    ...
    try {
      ...
      switch (err.code) {
        case 'auth/email-already-in-use': setError('Un compte existe déjà avec cet email.'); break
    ...
    <div className="min-h-screen bg-[#1A3A6B] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-white/50 text-sm">Créer un compte</p>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-xl">
          ...
          <div className="space-y-3">
            ...
            <Field label="Mot de passe *" value={password} onChange={setPassword} placeholder="Min. 6 caractères" type="password" />
            <Field label="Confirmer *" value={password2} onChange={setPassword2} placeholder="Répétez le mot de passe" type="password" />
          </div>
        ...
        <p className="text-white/20 text-xs text-center mt-6">VESTILIB · Inscription sécurisée</p>
      </div>
    </div>
  )
}
